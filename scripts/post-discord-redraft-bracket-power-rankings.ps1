param(
  ...
  [string]$ConfigPath = "data/discord-redraft-bracket-power-rankings-config.json",
  [string]$StatePath = "data/discord-redraft-bracket-power-rankings-state.json",
  [string]$WebhookUrl = $env:DISCORD_WEBHOOK_REDRAFT_BRACKET_POWER_RANKINGS,
  [switch]$ForceRefresh,
  [switch]$DryRun,
  [switch]$PassThru
)
...
if ($expectedLeagueIds.Count -ne 5 -or @($expectedLeagueIds | Sort-Object -Unique).Count -ne 5) {
  throw "Power-ranking config must contain five unique Redraft Bracket league IDs."
}
if ($expectedTeamCount -ne 60) { throw "Power-ranking config must expect exactly 60 teams." }
if ($topCount -ne 20) { throw "Power-ranking config must publish a combined Top 20." }
...
foreach ($leagueId in $expectedLeagueIds) {
  ...
  if ((Get-StringValue (Get-PropertyValue $matches[0] "format")).ToLowerInvariant() -ne "bracket") {
    throw "League '$leagueId' is not marked as Redraft Bracket data. Existing Discord rankings were left untouched."
  }
...
foreach ($leagueId in $expectedLeagueIds) {
  ...
  if (-not $allDraftsComplete -or -not $draftReady -or -not $publish -or $leagueRows.Count -ne 12) {
    $rankingsReady = $false
    continue
  }
...
if ($rankingsReady -and $combinedRows.Count -ne $expectedTeamCount) {
  throw "The refreshed Redraft Bracket rankings contain $($combinedRows.Count) teams instead of $expectedTeamCount. Existing Discord rankings were left untouched."
}
...
if (-not $rankingsReady -and $hasPublishedRankings) {
  throw "The refreshed ranking data is incomplete, so the last known good Discord Top 20 was preserved."
}
...
if ($rankingsReady) {
  ...
  foreach ($rangeStart in @(1, 6, 11, 16)) {
  ...
  $fields.Add(@{
    ...
    value = "After all five drafts finish, the 60 teams are graded together and the combined Top 20 replaces this card automatically."
    inline = $false
  }) | Out-Null
}

$embed = @{
  title = "Redraft Bracket - Power Rankings"
  url = $websiteUrl
  description = if ($rankingsReady) {
    "Combined Top 20 across Titan, Apex, Iron, Vanguard, and Dominion. Current roster-strength snapshot: $snapshotLabel."
  } else {
    "The combined Top 20 will activate only after all five division drafts are complete. Partial and sample rankings are never published."
  }
...
if (-not $ForceRefresh -and $messageId -and (Get-StringValue $state.signature) -eq $signature) {
  ...
  if ($PassThru) { $result } else { Write-Host "Redraft Bracket power rankings are already current for $snapshotKey." }
  exit 0
}
...
if ($DryRun) {
  ...
  if ($PassThru) { $result } else {
    Write-Host ("DRY RUN Redraft Bracket power rankings: ready: {0}; Top 20 rows: {1}." -f $rankingsReady, $topRows.Count)
    Write-Output ($payload | ConvertTo-Json -Depth 12)
  }
  exit 0
}

if ([string]::IsNullOrWhiteSpace($WebhookUrl)) { throw "DISCORD_WEBHOOK_REDRAFT_BRACKET_POWER_RANKINGS is not configured." }
...
if ($webhookChannelId -ne $channelId) {
  throw "The Redraft Bracket power-ranking webhook points to channel '$webhookChannelId', not configured channel '$channelId'."
}
...
if (-not $messageId) {
  ...
  if (-not $messageId) { throw "Discord did not return a message ID for the Redraft Bracket power-ranking card." }
  $action = "created"
}
...
if ($PassThru) {
  ...
  Write-Host ("Redraft Bracket power rankings {0}: ready: {1}; Top 20 rows: {2}; snapshot: {3}." -f $action, $rankingsReady, $topRows.Count, $snapshotKey)
}

param(
  ...
  [string]$SourcePath,
  [string]$RawOutputPath = "data/private/leaguesafe-bbu-current.csv",
  [string]$PaymentsOutputPath = "data/private/leaguesafe-payments.csv",
  [switch]$PassThru
)
...
if (-not [string]::IsNullOrWhiteSpace($rawOutputDirectory) -and -not (Test-Path -LiteralPath $rawOutputDirectory)) {
  New-Item -ItemType Directory -Path $rawOutputDirectory | Out-Null
}

$paymentsOutputDirectory = Split-Path -Parent $PaymentsOutputPath
if (-not [string]::IsNullOrWhiteSpace($paymentsOutputDirectory) -and -not (Test-Path -LiteralPath $paymentsOutputDirectory)) {
  New-Item -ItemType Directory -Path $paymentsOutputDirectory | Out-Null
}
...
$payments = @($rows | ForEach-Object {
  ...
  if ($paid -gt 0) {
    [pscustomobject]@{
      paymentId = "LS-BBU-$($_.OwnerId)"
      leagueGroup = "Best Ball Union"
      leagueRecordId = ""
      payerName = $_.Owner
...
if ($PassThru) {
  ...
  Write-Host "Saved raw BBU LeagueSafe export to $RawOutputPath"

...
The player and team calculations follow `docs/vbp-power-ranking-model.md`: the script applies every matching live Sleeper scoring coefficient to Sleeper's Week 1-17 stat projections before grading lineup strength, ceiling, and depth. It records applied scoring keys and any explicitly zero-assumed categories in each league's projection metadata. Core projection inputs fail closed when incomplete. Dynasty owner scores use a fixed within-league display scale from their calculated strength difference, rather than points assigned by rank. Positional boards rank owners at each available position without publishing individual player lists.

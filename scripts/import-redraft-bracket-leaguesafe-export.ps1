param(
  ...
  [string]$SourcePath,
  [string]$RawOutputPath = "data/private/leaguesafe-bracket-current.csv",
  [string]$PaymentsOutputPath = "data/private/leaguesafe-bracket-payments.csv",
  [switch]$PassThru
)
...
$payments = @($rows | ForEach-Object {
  ...
  if ($paid -gt 0) {
    [pscustomobject]@{
      paymentId = "LS-RDB-$($_.OwnerId)"
      leagueGroup = "Redraft Bracket"
      leagueRecordId = ""
      payerName = $_.Owner
...
if ($PassThru) {
  ...
  Write-Host "Saved raw Redraft Bracket LeagueSafe export to $RawOutputPath"

...
- Keep Facebook posts short, direct, and centered on what you are actively filling now.
- Separate `Bracket Redraft` and `Dynasty Bracket` instead of combining them into one ad.
- Keep `Best Ball Union` focused on `BBU1` first while the room is being built.
- Only use the `May 1` target and `draft set within 24 hours` language for `BBU`.
- When possible, lead with real format hooks like `Superflex`, `3RR`, or the actual up-front dynasty cost.

## April 16 2026 Active Push
...
```text
VBP Fantasy Network is building Best Ball Union starting with the first league. I cleared out the old inactive players, and this is the fresh room I am focused on filling first.

This is a $10 fast-draft best ball league on Sleeper. Best Ball Union also has league prizes and overall Union high-score prizes.
...
- On Facebook, keep the first sentence focused on one format instead of trying to sell the whole network at once.
- Keep the bracket formats separated unless you intentionally want a broad hub post.
- For `BBU`, keep saying `starting with the first league` until `BBU1` is stable or nearly full.
- Keep the `May 1` and `24 hours after everyone is paid` timing language exclusive to `BBU` unless your launch plan changes.

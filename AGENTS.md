...
- `assets/js/bestball-center.js`
  ...
  - Reads the generated `data/power-rankings.json` snapshot for a combined Top 20 power-ranking board across completed BBU draft rooms.
...
- `assets/js/power-rankings.js`
  ...
  - Reads `data/power-rankings.json` and renders clean owner ranking rows, verified scoring settings, owner-by-position rankings, and public methodology.
...
- Current-season power rankings must be scoring-derived, not merely scoring-aware: multiply Sleeper Week 1-17 projected stat components by every matching live `scoring_settings` coefficient before calculating lineup, ceiling, and depth. Record unsupported nonzero offensive categories as explicit zero assumptions, and fail closed if core passing, rushing, receiving, or positional-reception projection inputs are missing.
- Best Ball Union power rankings must use the combined custom-scoring projection grades across all configured rooms. Wins, points for, and weekly high scores must not change the roster-strength grade; points for may only break an exact published-score tie.
